        ...
        public static SettingsProvider CreateMyCustomSettingsProvider()
        {
            ...
            var provider = new SettingsProvider("Project/MyCustomIMGUISettings", SettingsScope.Project)
            {
                ...
                guiHandler = (searchContext) =>
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    if (GUILayout.Button(new GUIContent("Fix All"), AVRO_Styles.TicketButton))
                    {
                    ...
                    // {
                    //     PopupWindow.ShowPopupRestoreWindow();
                    // }
                    EditorGUILayout.EndHorizontal();
...
#pragma warning restore UDR0001 // Domain Reload Analyzer
        ...
        void GetSelection(AVRO_Ticket _ticket)
        {
            ...
            else if ((ticket.data.tags & AVRO_Settings.TicketTags.Files) == AVRO_Settings.TicketTags.Files)
            {
                for (int i = 0; i < _ticket.data.concernedObjects.Count; i++)
                {
                    if ((ticket.data.tags & AVRO_Settings.TicketTags.Textures) == AVRO_Settings.TicketTags.Textures)
                        _selection.Add(AssetDatabase.LoadAssetAtPath<Texture2D>(_ticket.data.concernedObjects[i].guid));
                    else if ((ticket.data.tags & AVRO_Settings.TicketTags.Materials) == AVRO_Settings.TicketTags.Materials)
                        _selection.Add(AssetDatabase.LoadAssetAtPath<Material>(_ticket.data.concernedObjects[i].guid));
                    else if ((ticket.data.tags & AVRO_Settings.TicketTags.Meshes) == AVRO_Settings.TicketTags.Meshes)
                        _selection.Add(AssetDatabase.LoadAssetAtPath<Mesh>(_ticket.data.concernedObjects[i].guid));
                }
            }
...
#pragma warning restore UDR0001 // Domain Reload Analyzer
        ...
        internal static void ShowPopupRestoreWindow()
        {
            settingsInstance = AVRO_Settings.GetOrCreateSettings();
            if (!settingsInstance.PopupWindow)
                settingsInstance.PopupWindow = CreateInstance<PopupWindow>();

            settingsInstance.PopupWindow.InitializeRestore();
            settingsInstance.PopupWindow.titleContent = new GUIContent("Restore to Last Fix All State");
            settingsInstance.PopupWindow.minSize = new Vector2(600, 150);
            settingsInstance.PopupWindow.maxSize = new Vector2(600, 150);
            settingsInstance.PopupWindow.ShowAuxWindow();
            settingsInstance.PopupWindow.Focus();
        ...
        void OnButtonFixAll()
        {
            ...
            for (int i = 0; i < _ticketsProperty.arraySize; i++)
            {
                var ticketProperty = _ticketsProperty.GetArrayElementAtIndex(i);
                AVRO_Ticket ticketInstance = ticketProperty.objectReferenceValue as AVRO_Ticket;
                if (!ticketInstance.data.hide && !ticketInstance.data.noFix && ticketInstance.data.state == AVRO_Settings.TicketStates.Todo && AVRO_Utilities.FixTicket(ticketInstance))
                {
                    ticketInstance.data.state = AVRO_Settings.TicketStates.Done;
                    EditorUtility.SetDirty(ticketInstance);
                }
            }
        ...
        void OnButtonFixAllButExperimentals()
        {
            ...
            for (int i = 0; i < _ticketsProperty.arraySize; i++)
            {
                var ticketProperty = _ticketsProperty.GetArrayElementAtIndex(i);
                AVRO_Ticket ticketInstance = ticketProperty.objectReferenceValue as AVRO_Ticket;
                if ((ticketInstance.data.tags & AVRO_Settings.TicketTags.Conditional) != AVRO_Settings.TicketTags.Conditional &&
                    !ticketInstance.data.hide && !ticketInstance.data.noFix && ticketInstance.data.state == AVRO_Settings.TicketStates.Todo && AVRO_Utilities.FixTicket(ticketInstance))
                {
                    ticketInstance.data.state = AVRO_Settings.TicketStates.Done;
                    EditorUtility.SetDirty(ticketInstance);
                }
